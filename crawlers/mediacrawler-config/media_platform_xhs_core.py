...
class XiaoHongShuCrawler(AbstractCrawler):
    ...
    async def start(self) -> None:
        ...
        async with async_playwright() as playwright:
            ...
            await self.context_page.goto(self.index_url, timeout=90000)
            ...
            if not await self.xhs_client.pong():
                ...
                await self._save_cookies_to_output()
    ...
    async def _save_cookies_to_output(self) -> None:
        """로그인 후 browser_context cookie 전체를 cn_social_listening/output/에 저장.

        - xhs_cookie.txt   : "name=value; name=value; ..." (단일 라인, 그대로 재사용 가능)
        - xhs_session.json : 메타데이터 + 전체 cookie 객체 (감사/만료 추적용)
        """
        try:
            cookies_list = await self.browser_context.cookies()
        except Exception as e:
            utils.logger.error(f"[XiaoHongShuCrawler._save_cookies] failed to read cookies: {e}")
            return

        cookie_str = "; ".join(f"{c['name']}={c['value']}" for c in cookies_list)
        # cwd = MediaCrawler/, project root = ../../ (cn_social_listening/)
        out_dir = os.path.abspath(os.path.join(os.getcwd(), "..", "..", "output"))
        os.makedirs(out_dir, exist_ok=True)

        txt_path = os.path.join(out_dir, "xhs_cookie.txt")
        json_path = os.path.join(out_dir, "xhs_session.json")
        ...
        meta = {
            "source": "MediaCrawler",
            "saved_at": _dt_now.now().isoformat(),
            "login_type": config.LOGIN_TYPE,
            "cookie_count": len(cookies_list),
            "has_a1": any(c.get("name") == "a1" for c in cookies_list),
            "has_web_session": any(c.get("name") == "web_session" for c in cookies_list),
            "has_gid": any(c.get("name") == "gid" for c in cookies_list),
        ...
        utils.logger.info(
            f"[XiaoHongShuCrawler] cookies saved → {txt_path} "
            f"({len(cookies_list)} cookies, web_session={meta['has_web_session']}, a1={meta['has_a1']})"
        )
