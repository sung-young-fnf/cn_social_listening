...
class XiaoHongShuClient(AbstractApiClient, ProxyRefreshMixin):
    ...
    async def request(self, method, url, **kwargs) -> Union[str, Any]:
        ...
        if response.status_code == 471 or response.status_code == 461:
            ...
            utils.logger.error(
                "[CAPTCHA-DIAG] url=%s status=%s Verifytype=%s Verifyuuid=%s",
                url, response.status_code, verify_type, verify_uuid,
            )
            utils.logger.error("[CAPTCHA-DIAG] response_headers=%s", resp_headers)
            utils.logger.error("[CAPTCHA-DIAG] response_body=%s", body_preview)
    ...
    async def _fetch_via_page(
        self, uri: str, params: Optional[Dict] = None, host: Optional[str] = None
    ) -> Dict:
        """page.evaluate fetch로 호출 — httpx 우회.

        Chromium 네트워크 스택을 거치므로 TLS/HTTP2/fingerprint가 진짜 Chrome과 동일.
        WAF가 봇 의심으로 461 캡차를 거는 user_posted 같은 엔드포인트 우회 목적.

        - sign은 xhshow로 만들어서 헤더에 박음 (브라우저 axios 인터셉터를 안 거치므로)
        - 쿠키는 credentials:'include'로 세션 자동 동봉
        """
        if params is None:
            params = {}
        signs = sign_with_xhshow(
        ...
        query = self._build_query_string(params) if params else ""
        full_url = f"{target_host}{uri}?{query}" if query else f"{target_host}{uri}"
        headers = {
            "x-s": signs["x-s"],
            "x-t": signs["x-t"],
            "x-s-common": signs["x-s-common"],
        ...
        """
        result = await self.playwright_page.evaluate(js, [full_url, headers])

        if result.get("error"):
            raise DataFetchError(f"page-fetch error: {result['error']}")
        ...
        if status in (461, 471):
            utils.logger.error(
                "[_fetch_via_page] CAPTCHA still on page fetch (status=%s) url=%s body=%s",
                status, full_url, result.get("text_preview"),
            )
            raise Exception(
                f"CAPTCHA via page-fetch (status={status}) — 옵션 2(수동 풀이)로 전환 고려"
            )

        if not body:
            raise DataFetchError(
                f"empty body (status={status}): {result.get('text_preview')}"
            )

        if body.get("success"):
            return body.get("data", {})
        if body.get("code") == self.IP_ERROR_CODE:
            raise IPBlockError(self.IP_ERROR_STR)
        if body.get("code") in (self.NOTE_NOT_FOUND_CODE, self.NOTE_ABNORMAL_CODE):
            raise NoteNotFoundError(f"Note not found, code: {body.get('code')}")
        raise DataFetchError(body.get("msg") or str(body))
    ...
    async def get_notes_by_creator(
        ...
        # xsec_token/xsec_source는 첫 호출에선 보통 없음. 빈 값 명시는 WAF 봇 시그널.
        # xhs 라이브러리처럼 응답에서 xsec_token 받아 후속 호출에 사용.
        params = {
            "num": page_size,
            "cursor": cursor,
            "user_id": creator,
        ...
        # WAF 우회: httpx 대신 로그인된 page에서 fetch (Chromium native fingerprint)
        return await self._fetch_via_page(uri, params)
